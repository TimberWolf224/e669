...
//  document.getElementById("fullsizeButton").setAttribute("href", fileUrl);
  ...
  if (currentApi == "e621") {
        ...
        if (result["source_url"]) {
            modalMetadata.innerHTML +=
                "<a class='btn-small blue' style='margin-right: 10px;' href='" + result["source_url"] + "'>" + "1" + "</a>";
        } else {
            modalMetadata.innerHTML += "(none)";
        }
    }
...
function refreshSettings() {
    ...
    // these use JSON.parse to make sure it's correctly evaluated as a boolean
    if (checkCookie("settings-verbose")) {verboseOutput = JSON.parse(getCookie("settings-verbose"));}
    if (checkCookie("settings-horizontal")) {horizontalOrder = JSON.parse(getCookie("settings-horizontal"));}
    if (checkCookie("settings-r18")) {r18 = JSON.parse(getCookie("settings-r18"));}
    if (checkCookie("settings-pagesize")) {pageSize = parseInt(getCookie("settings-pagesize"));}
    if (checkCookie("settings-derpiapikey")) {derpiApiKey = getCookie("settings-derpiapikey");}
